...
* Tab temporarily toggles "quick preview" mode for the current session: when active, hovering or arrowing to an item will automatically display a hover preview for it, positioned so that it's always *outside* the menu (unless you're so deep in subfolders you've reached the edge of your screen).  This makes it really easy to browse the contents of a folder just by arrowing down through it.  The "Automatic preview" setting controls the default behavior; when automatic preview is off, pointer previews require Ctrl/Cmd.
* If "Preview selected item while holding Ctrl/Cmd" is enabled, holding Ctrl/Cmd while navigating with the keyboard previews the currently selected file or folder note without moving the mouse whenever automatic preview is off.
